    ...
    if (fwrite || ftest)
    {
        ...
        if (PRIM->TME)
        {
            ...
            if (GSLocalMemory::m_psm[context->TEX0.PSM].pal > 0)
            {
                ...
                const u32 pal_size = GSLocalMemory::m_psm[context->TEX0.PSM].pal;
                gd.clut = (u32*)m_vertex_heap.alloc(sizeof(u32) * pal_size, VECTOR_ALIGNMENT);

                memset(gd.clut, 0, sizeof(u32) * pal_size);
                memcpy(gd.clut, (const u32*)m_mem.m_clut, sizeof(u32) * pal_size);
            }
...
void GSRendererSW::SharedData::UpdateSource()
{
    ...
    if (GSConfig.SaveTexture && GSConfig.ShouldDump(s_n, g_perfmon.GetFrame()))
    {
        ...
        if (global.clut)
        {
            ...
            const u32 pal_size = GSLocalMemory::m_psm[g_gs_renderer->m_context->TEX0.PSM].pal;
            GSPng::Save((IsDevBuild || GSConfig.SaveAlpha) ? GSPng::RGB_A_PNG : GSPng::RGB_PNG, s, reinterpret_cast<const u8*>(global.clut), pal_size, 1, sizeof(u32) * pal_size, GSConfig.PNGCompressionLevel, false);
        }
    }
}
