...
//	xMOVSS(ptr32[&mVU.regs().VI[REG_Q].UL], xmmPQ);
    armAsm->Str(xmmPQ.S(), armMemOperandPtr(&mVU.regs().VI[REG_Q].UL));
//	xPSHUF.D(xmmPQ, xmmPQ, 0xe1);
    armPSHUFD(xmmPQ, xmmPQ, 0xe1);
//	xMOVSS(ptr32[&mVU.regs().pending_q], xmmPQ);
    armAsm->Str(xmmPQ.S(), armMemOperandPtr(&mVU.regs().pending_q));
//	xPSHUF.D(xmmPQ, xmmPQ, 0xe1);
    armPSHUFD(xmmPQ, xmmPQ, 0xe1);
...
//		xMOVSS(ptr32[&mVU.regs().VI[REG_P].UL], xmmPQ);
        armAsm->Str(xmmPQ.S(), armMemOperandPtr(&mVU.regs().VI[REG_P].UL));
//		xPSHUF.D(xmmPQ, xmmPQ, 0x87); // 0 2 1 3
        armPSHUFD(xmmPQ, xmmPQ, 0x87);
//		xMOVSS(ptr32[&mVU.regs().pending_p], xmmPQ);
        armAsm->Str(xmmPQ.S(), armMemOperandPtr(&mVU.regs().pending_p));
...
//	xMOV(ptr32[&mVU.regs().VI[REG_STATUS_FLAG].UL], gprT1);
    armAsm->Str(gprT1, armMemOperandPtr(&mVU.regs().VI[REG_STATUS_FLAG].UL));
    mVUallocMFLAGa(mVU, gprT1, fMac);
    mVUallocCFLAGa(mVU, gprT2, fClip);
//	xMOV(ptr32[&mVU.regs().VI[REG_MAC_FLAG].UL], gprT1);
    armAsm->Str(gprT1, armMemOperandPtr(&mVU.regs().VI[REG_MAC_FLAG].UL));
//	xMOV(ptr32[&mVU.regs().VI[REG_CLIP_FLAG].UL], gprT2);
    armAsm->Str(gprT2, armMemOperandPtr(&mVU.regs().VI[REG_CLIP_FLAG].UL));
...
//		xMOVAPS(ptr128[&mVU.regs().micro_macflags], xmmT1);
        armAsm->Str(xmmT1.Q(), armMemOperandPtr(&mVU.regs().micro_macflags));
//		xMOVAPS(xmmT1, ptr128[mVU.clipFlag]);
        armAsm->Ldr(xmmT1.Q(), armMemOperandPtr(mVU.clipFlag));
//		xMOVAPS(ptr128[&mVU.regs().micro_clipflags], xmmT1);
        armAsm->Str(xmmT1.Q(), armMemOperandPtr(&mVU.regs().micro_clipflags));
...
//		xMOVDZX(xmmT1, ptr32[&mVU.regs().VI[REG_CLIP_FLAG].UL]);
        armAsm->Ldr(xmmT1, armMemOperandPtr(&mVU.regs().VI[REG_CLIP_FLAG].UL));
...
//		xMOVDZX(xmmT1, ptr32[&mVU.regs().VI[REG_MAC_FLAG].UL]);
        armAsm->Ldr(xmmT1, armMemOperandPtr(&mVU.regs().VI[REG_MAC_FLAG].UL));
//		xSHUF.PS(xmmT1, xmmT1, 0);
        armSHUFPS(xmmT1, xmmT1, 0);
//		xMOVAPS(ptr128[&mVU.regs().micro_macflags], xmmT1);
        armAsm->Str(xmmT1.Q(), armMemOperandPtr(&mVU.regs().micro_macflags));
...
//		xMOVAPS(ptr128[&mVU.regs().micro_statusflags], xmmT1);
        armAsm->Str(xmmT1.Q(), armMemOperandPtr(&mVU.regs().micro_statusflags));
    }

	if (EmuConfig.Gamefixes.VUSyncHack || EmuConfig.Gamefixes.FullVU0SyncHack) {
//        xMOV(ptr32[&mVU.regs().nextBlockCycles], 0);
        armAsm->Str(a64::wzr, armMemOperandPtr(&mVU.regs().nextBlockCycles));
    }

//	xMOV(ptr32[&mVU.regs().VI[REG_TPC].UL], xPC);
    armStorePtr(xPC, &mVU.regs().VI[REG_TPC].UL);
...
//			xAND(ptr32[&VU0.VI[REG_VPU_STAT].UL], (isVU1 ? ~0x100 : ~0x001)); // VBS0/VBS1 flag
            armAnd(&VU0.VI[REG_VPU_STAT].UL, (isVU1 ? ~0x100 : ~0x001));
		}
	}
...
void mVUendProgram(mV, microFlagCycles* mFC, int isEbit)
{
	...
	if (isEbit && isEbit != 3)
	{
		std::memset(&mVUinfo, 0, sizeof(mVUinfo) + sizeof(mVUregsTemp));
		mVUincCycles(mVU, 100); // Ensures Valid P/Q instances (And sets all cycle data to 0)
		mVUcycles -= 100;
		qInst = mVU.q;
		pInst = mVU.p;
...
//		xMOV(ptr32[&mVU.regs().VI[REG_TPC].UL], arg1regd);
        armAsm->Str(EAX, armMemOperandPtr(&mVU.regs().VI[REG_TPC].UL));
		if (mVU.index && THREAD_VU1) {
//            xFastCall((void *) mVUEBit);
            armEmitCall(reinterpret_cast<void*>(mVUEBit));
        }
...
//		xMOV(gprT1, ptr32[&mVU.branch]);
        armAsm->Ldr(gprT1, armMemOperandPtr(&mVU.branch));
//		xMOV(ptr32[&mVU.regs().VI[REG_TPC].UL], gprT1);
        armAsm->Str(gprT1, armMemOperandPtr(&mVU.regs().VI[REG_TPC].UL));
//		xJMP(mVU.exitFunct);
        armEmitJmp(mVU.exitFunct);
//		eJMP.SetTarget();
        armBind(&eJMP);
